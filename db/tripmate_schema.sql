-- 0) DB가 없으면 생성
CREATE DATABASE IF NOT EXISTS tripmate DEFAULT CHARACTER SET utf8mb4 DEFAULT COLLATE utf8mb4_general_ci;
-- 사용할 DB
USE tripmate;
-- Users: 사용자 기본 정보
...
CREATE TABLE IF NOT EXISTS Place (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT fk_place_category FOREIGN KEY (category_id) REFERENCES PlaceCategory(category_id) ON UPDATE CASCADE ON DELETE RESTRICT,
...
CREATE TABLE IF NOT EXISTS Trip (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT fk_trip_user FOREIGN KEY (user_id) REFERENCES Users(user_id) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT fk_trip_region FOREIGN KEY (region_id) REFERENCES Region(region_id) ON UPDATE CASCADE ON DELETE
  SET NULL,
...
CREATE TABLE IF NOT EXISTS TripDay (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT fk_tripday_trip FOREIGN KEY (trip_id) REFERENCES Trip(trip_id) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE IF NOT EXISTS ScheduleItem (
  ...
  created_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP,
  updated_at DATETIME NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  CONSTRAINT fk_scheduleitem_tripday FOREIGN KEY (trip_day_id) REFERENCES TripDay(trip_day_id) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT fk_scheduleitem_place FOREIGN KEY (place_id) REFERENCES Place(place_id) ON UPDATE CASCADE ON DELETE
  SET NULL,
